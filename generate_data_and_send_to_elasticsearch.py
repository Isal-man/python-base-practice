...
# Initialize the first chunk and send to Elasticsearch
df = generate_chunk(chunk_size)
bulk_index_to_elasticsearch(df, index_name)

# Append subsequent chunks with progress bar
total_chunks = total_rows // chunk_size

for _ in tqdm(range(total_chunks - 1), desc="Generating data", unit="chunk", unit_scale=True, ncols=100):
    df = generate_chunk(chunk_size)
    bulk_index_to_elasticsearch(df, index_name)
